# модель бинарного дерева c методами обхода узлов
...
class BST:
    ...
    def WideAllNodes(self):
        '''Метод обхода дерева в ширину, начиная с корня
        return tuple(nodes)
        '''
        pointers = []
        queue = [self.Root]
        while len(queue) > 0:
            node = queue.pop()  # удаляет узел и сохраняет его в переменной
            pointers.append(node)  # добавляет узел в список
            '''пополнение очереди потомками, если они имеются'''
            if node.LeftChild:
    ...
    def DeepAllNodes(self, form):
        '''
        Метод обхода дерева в глубину, начиная с корня
        form - порядок обхода: 0(in_order), 1(post_order), 2(pre_order)/
        return tuple(nodes)
        '''
        if form == 0:
    ...
    def post_order(self, node):
        '''Обход узлов: левый, правый, корень'''
        if node.LeftChild:
    ...
    def pre_order(self, node):
        '''Обход узлов: корень, левый, правый'''
        yield node
        if node.LeftChild:
